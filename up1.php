...
if(!isset($_SESSION["loginMember"]) || ($_SESSION["loginMember"]=="")){
				echo "<script type='text/javascript'>alert('尚未登入，請先從右側選擇登入會員');location.href = 'index.php';</script>";
		 }
...
$_SESSION["idf"]=555;
 ...
 $foodidsql="SELECT MAX(imageId) FROM `output_images`";
	 ...
	 echo $foodid[0];
	 $_SESSION["idf"]=$foodid[0];
		echo $_SESSION["idf"];

  }
  header("Location: up2.php?id=$id");
  ...
  <main role="main" class="inner cover">
    ...
    <div class="container" >
      ...
      <form class="form-signin" enctype="multipart/form-data" name="form1" method="Post"  >
		...
		?>
        商店店名:<label for="shopname" class="sr-only">商店店名</label>
        <input type="text" name="shop" id="inputshop" class="btn" value='<?php echo $rs[4]?>' required autofocus>
        <br><br>
		電話:<label for="phone" class="sr-only">電話</label>
        <input type="text" name="fon" id="inputfon" class="btn" value='<?php echo $rs[5]?>' required autofocus>
        
        <br><br>地址:<label for="address" class="sr-only">地址</label>
        <input type="text" name="adds" id="inputadds" class="btn" value='<?php echo $rs[6]?>' required autofocus>
        <br><br>營業時間:<label for="time" class="sr-only">營業時間</label>
       <input type="text" name="tim" id="inputtim" class="btn" value='<?php echo $rs[7]?>' required autofocus>

        <br><br>簡介:<label for="allth" class="sr-only">簡介</label>
        <br><input type="text" name="allth" id="inputallth" class="form-control" value="<?php echo $rs[8];?>" required autofocus>
       <br><br> 平均消費:<label for="dollar" class="sr-only">平均消費</label>
        <input type="text" name="dollar" id="inputdollar" class="btn" value='<?php echo $rs[11]?>' required autofocus>
        <br><br>
        上傳商家圖片:
...
if (document.all.inputtype.selectedIndex == 1)
 ...
 document.all.type1.value = 3 ;
       ...
       <button class="btn btn-lg btn-primary btn-block" name="Submit" type="submit" onclick="submit" >申請更新</button>
 ...
 if (isset($_POST['Submit'])) {
     ...
     $sql  = "SELECT * from `memberdata` WHERE `m_name` = '{$row_RecMember["m_name"]}'";
	 $m =$conn->query($sql);
	 $r=$m->fetch_assoc();
	 ...
	 {
		 echo  $r["m_time"];
         echo "<script type='text/javascript'>alert('每日申請次數達上限');location.href = 'index.php';</script>";
	 }
	 else {
	 ...
	 if ($_FILES["shopim"]["error"]==0){
		if (move_uploaded_file($_FILES["shopim"]["tmp_name"],"picture/".$_FILES["shopim"]["name"])){
			$_SESSION["ids"]=$_FILES["shopim"]["name"];
		
		} 
	 }
	 else {echo "<script type='text/javascript'>alert('圖片上傳失敗');</script>";
	       $checksh++;
	 }
	 
	 if ($_FILES["foodim"]["error"]==0 ){
		if (move_uploaded_file($_FILES["foodim"]["tmp_name"],"picture/".$_FILES["foodim"]["name"])){
			$_SESSION["idf"]=$_FILES["foodim"]["name"];
	 ...
	 if ( $checksh==0 && $checkfo==0 )
	    {
			echo " <script>location.href = 'up2.php?id=$id';</script>";;
		}
	 else {
		 $checksh=0;
		 $checkfo=0;
